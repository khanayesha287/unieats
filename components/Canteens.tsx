...
const featuredCanteens = canteens.slice(0, 1);
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto grid max-w-md grid-cols-1 justify-items-center gap-8">
          {featuredCanteens.map((canteen) => (
            <Link
              key={canteen.slug}
              href={`/menu/${canteen.slug}`}
              className="group flex w-full flex-col overflow-hidden rounded-3xl border border-transparent bg-white shadow-lg shadow-[#6C2BD9]/5 transition-all duration-300 hover:-translate-y-2 hover:border-[#6C2BD9]/20 hover:shadow-xl hover:shadow-[#6C2BD9]/15"
            >
              <div className={`relative h-44 overflow-hidden border-t-4 border-[#6C2BD9] ${canteen.image ? '' : `bg-gradient-to-br ${canteen.gradient}`}`}>
                ...
                <span className="absolute right-4 top-4 rounded-full bg-[#F4C542] px-3 py-1 text-xs font-bold uppercase tracking-wide text-[#2E1065] shadow-sm transition-shadow group-hover:shadow-[0_0_12px_rgba(244,197,66,0.6)]">
                  OPEN
                </span>
              </div>

              <div className="flex flex-1 flex-col p-6">
                <h3 className="text-xl font-bold text-gray-900 transition-colors group-hover:text-[#6C2BD9]">{canteen.name}</h3>
